...
void (*_audio_pcm_clk_state_callback)(void *, int);
void (*_audio_dha_callback)(void *, dha_data_t*);
...
void ril_register_pcm_clk_state_callback(void *function, void *data)
{
    _audio_pcm_clk_state_callback = function;
    callback_data = data;
}

void ril_register_dha_callback(void *function, void *data)
{
    _audio_dha_callback = function;
    callback_data = data;
}

/* This is the callback function that the RIL uses to
set the wideband AMR state */
static int onSetWbAMRCallback(void *ril_client, const void *data, size_t datalen)
{
...
static int onUnsolPcmClkStateCallback(void *ril_client, const void *data, size_t datalen)
{
    int enable = ((int *)data)[0];

    if (!callback_data || !_audio_pcm_clk_state_callback)
        return -1;

    _audio_pcm_clk_state_callback(callback_data, enable);

    return 0;
}

static int onUnsolDHACallback(void *ril_client, const void *data, size_t datalen)
{
    if (!callback_data || !_audio_dha_callback)
        return -1;

    _audio_dha_callback(callback_data, (dha_data_t *)data);
...
static int ril_connect_if_required(struct ril_handle *ril)
{
    ...
    if(_ril_get_wb_amr)
        _ril_get_wb_amr(ril->client, onSetWbAMRCallback);
...
int ril_open(struct ril_handle *ril)
{
    ...
    _ril_register_unsolicited_handler(ril->client, RIL_UNSOL_WB_AMR_STATE, onSetWbAMRCallback);
    _ril_register_unsolicited_handler(ril->client, RIL_UNSOL_PCMCLOCK_STATE, onUnsolPcmClkStateCallback);
    _ril_register_unsolicited_handler(ril->client, RIL_UNSOL_DHA_STATE, onUnsolDHACallback);
...
int ril_set_loopback(struct ril_handle *ril, int mode, int secType) {
    if (ril_connect_if_required(ril))
        return 0;

    if (mode > 1) {
        ALOGE("Invalid loopback mode. Fail to set loopback.");
        return -1;
    }
    
    ALOGI("SetLoopbackTest off ret %d, mode %d", result, mode);
    result = SetLoopbackTest(this->rilClientActive, mode, this->getDeviceType(secType));
